import (
	...
	"net/http"
	"os-admin/admin-user/api/internal/common"
	"os-admin/admin-user/common/errorx"
	"os-admin/admin-user/common/response"
	"os-admin/admin-user/enum"
	"slices"
	"strings"
)

// 权限校验中间件
var ErrInvalidToken = errorx.NewCodeError(400, enum.LoginOutOfDate)
var ErrInvalidResource = errorx.NewCodeError(-1, enum.ResourceNotAuth)
var ErrInvalidAuth = errorx.NewCodeError(-1, enum.UserAuthNotEnough)
...
func (m *AuthMiddleware) Handle(next http.HandlerFunc) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//没有资源权限的话直接返回错误
		if len(roleNames) < 1 {
			response.Response(w, nil, ErrInvalidAuth)
			return
		}
